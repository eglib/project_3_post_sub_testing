...
int   window_size;         // Size of the Go-Back-N window.
...
int main(int argc, char* argv[]) {
  ...
  // window size  : Number of packets in the go-back-n window.
  // input file   : Path to file with contents to be transmitted over simulated network.

  if (argc != 8) {
    printf("Error: Incorrect number of command line arguments\n");
    printf("usage: %s <loss prob> <corrupt prob> <pkt interval> <seed> <debug> <window size> <input file>\n", argv[0]);
    exit(-1);
  }
  ...
  while (1) {
    ...
    if (eventptr->evtype == FROM_LAYER5 ) {
      ...
      // Copy up to the next 32 bytes of the input file into the message.
      size_t bytes_read = fread(msg2give.data, 1, 32, tx_file);
      msg2give.length = bytes_read;
      if (bytes_read == 32 && feof(tx_file) == 0) {
...
void tolayer3(int AorB, struct pkt packet) {
  ...
  // Finally, compute the arrival time of packet at the other end. medium can
  // not reorder, so make sure packet arrives between 1 and 10 time units after
  // the latest arrival time of packets currently in the medium on their way to
  // the destination.
  lastime = time;
  for (q=evlist; q!=NULL; q = q->next) {
  ...
  if (jimsrand() < corruptprob)  {
    ...
    if (x < .65) {
      ...
      // Corrupt checksum value.
      if (x < 0.7) {
        mypktptr->checksum += ((int) (x*10));
      } else {
        mypktptr->checksum = 77744;
      }

    } else if (x < .85) {
      // Corrupt sequence number.
      if (x < 0.8) {
        mypktptr->seqnum += ((int) (x*10));
      } else {
        mypktptr->seqnum = 999999;
      }

    } else if (x < .925) {
      // Corrupt acknowledgment number.
      if (x < 0.88) {
        mypktptr->acknum += ((int) (x*10));
      } else {
